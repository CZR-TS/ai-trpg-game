...
`opening_background` 会原样展示给玩家，因此它既不是一句宣传语，也不是给 AI 阅读的幕后提纲。每一本新世界书都应按以下标准编写：

1. 建议写成 **4-6 个自然段、至少约 500 个中文字符**；体量复杂的世界可以更长，但不要靠辞藻和同义复述凑字数。
2. 应让第一次接触该世界的玩家理解：主要地理区域、种族或社会结构、力量/能力体系、重要历史、主要势力、当前时代危机，以及普通人在这个世界中的生存处境。
3. 只写所有新游戏都成立、角色在进入世界前可以知道的公开背景。玩家姓名、角色经历、某一局的开场事件要等角色创建后由 AI 个性化生成。
4. 隐藏真相、秘密势力计划、内部状态值和幕后剧情标志不能直接写进开场；它们应放在关键词条目、常驻条目或后续 `story_state.flags` 中。
5. 可以使用 `\n\n` 自然分段，并用 `**重点文字**` 少量标记核心专有名词，禁止整段加粗。

编写完成后，应检查开场能否独立回答“这是哪里、谁在统治、力量如何运作、过去发生了什么、现在为什么危险、普通人如何生活”。玄幻示例的 `opening_background` 是完整度参考，不代表其他题材必须照搬五域、修炼境界或魔渊等具体设定。

### 系统提示词与玩家正文边界（所有世界通用）

项目的全局系统提示词会对所有世界书统一强制以下规则，换世界书时也不得取消：

- 剧情合理、身份一致和因果连续高于制造冲突或强行推进。角色的能力、物品、关系、权限、位置和已知信息必须来自角色资料、世界设定、结构化状态或已经发生的剧情，不能临时编造。
- 玩家写的是行动意图，不代表必然成功；条件不足时应合理失败、部分完成或产生代价。新人物、新物品和新线索必须有成立的来源，禁止突然救援、无铺垫巧合、隐藏血统、临时神器或角色降智。
- `narrative` 通常为 3-5 个自然段、约 450-800 个中文字符，内容用于实际事件、人物反应、可观察细节、因果和有效线索。
- `summary` 通常为 3-4 个自然段、约 300-550 个中文字符，交代行动过程、直接结果、代价、人物反应和新线索。
- `narrative`、`summary`、`intro.world`、`ending.text` 是玩家正文，只能写角色可以看到、听到、感知、推断或从他人处得知的内容。
- “互信建立、关系提升、共同目标更新、探索方向出现、内部状态变化、进度达到”等后台结论，不得以系统总结口吻直接展示给玩家。关系、目标、方向、内部数值和幕后标志应写入 `story_state.shared`、角色状态或 `flags`。
- 如果后台变化需要让玩家感受到，必须表现为具体的对话、动作、物证、环境变化或角色可得出的线索。
- 增加篇幅不是增加空话；禁止用空泛气氛、同义形容、重复解释或幕后旁白凑字数。

每个世界可以通过配套 `dm_character.json` 增加题材风格、特有规则和随机事件策略，但不得覆盖或削弱上述通用规则。创建新角色卡时复制 `worldbook/dm_character.template.json`；不要把完整固定背景重复塞进角色卡，否则会浪费上下文并降低缓存稳定性。
...
4. **验证**：用 `server/lorebook.js` 的 `activateEntries` 跑几个真实场景（开场、进城、进秘境），检查：
   ...
   - 单字词没有引发大面积误触发。
5. **配 DM 角色卡**：复制 `worldbook/dm_character.template.json`，在 `system_prompt` 中补充本世界的题材风格、特有规则和随机事件策略；不得覆盖全局正文长度、玩家正文边界与输出协议。
...
可参考 `worldbook/template.json`（世界书空模板）、`worldbook/dm_character.template.json`（配套DM角色卡模板）与 `worldbook/examples/xuanhuan-example/worldbook.json`（完整示例，202 条，覆盖全部机制用法）。
