...
assert.deepEqual(game.roomChatReadState(room), { A: 0, B: 0 }, '新房间双方已读游标必须从零开始');
assert.equal(game.markRoomChatRead(room, 'B'), chatMessage.createdAt, '玩家读取聊天后必须推进到最新一条对方消息');
assert.equal(game.roomChatReadState(room).B, chatMessage.createdAt, '服务端必须保留玩家 B 的已读位置');
...
assert.equal(chatSnapshot.chatReadAt.B, chatMessage.createdAt, '聊天已读位置必须写入进行中房间磁盘快照');
game.clearRoomChat(room);
assert.deepEqual(game.roomChatHistory(room), [], '房间结束清理必须能移除全部聊天记录');
assert.deepEqual(game.roomChatReadState(room), { A: 0, B: 0 }, '清理聊天记录时必须同时清除已读位置');
...
assert.match(socketServer, /chat:read[\s\S]*markRoomChatRead[\s\S]*emit\('chat:read'/, '服务端必须持久化已读游标并同步给双方');
assert.match(socketServer, /game:started[\s\S]*emitRoomState\(room\)[\s\S]*game:intro/, '游戏开始后必须立即同步 playing 状态，避免聊天提醒出现空窗');
