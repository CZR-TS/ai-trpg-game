...
- **AI 叙事 DM**：调用 OpenAI 兼容大模型（GLM / DeepSeek / GPT 等均可）作为主持人，支持在玩家阅读反馈时预生成下一回合。
- **固定世界背景**：同一世界的初始背景由世界书 `opening_background` 直接提供；玩家完成角色塑造后，AI 再生成个性化第一回合，避免每局重复生成相同内容。
...
- 所有世界书都必须填写非空的 `opening_background`；更新固定开场背景时直接修改该顶层字段，具体规则见 `worldbook/README.md`。
- 新世界书的固定开场需说明地理、社会/种族、力量体系、历史、势力、当前危机与普通人生存处境；配套DM角色卡请复制 `worldbook/dm_character.template.json`。玩家正文与后台状态的统一边界、回合信息量要求见 `worldbook/GUIDE.md`，更换世界书时同样生效。
...
- 更新方式：开发机使用 `git archive` 生成带目录前缀的发布包，经 SCP 直传服务器，再由 `update.sh` 从本地压缩包原子更新；服务器不访问 GitHub。
...
- [x] **开局角色塑造**：世界观背景展示之后，玩家可填写个人资料（性别、性格等，可选填或自由输入），并支持重新设置昵称——入场时填写的昵称仅作为身份标识，开局后可改名，此后在剧情中以新的个人资料示人。
- [x] **P0 · DM 真实生成状态**：通过 DeepSeek SSE 流实时展示连接、推演、内容接收、结构校验与重试状态，并显示实际接收字符数和完整结构项；玩家页只显示三组数据：上下文窗口/当前占用、本次 Token（括号内为本次缓存 Token）、本局总缓存率。
- [x] **叙事排版优化**：AI 输出的世界观背景与剧情叙事适当分段（按场景/段落自然断行），并对重点文字（关键词、专有名词、强调内容）加粗显示，前端渲染支持 Markdown 粗体。
- [x] **中度简洁文风**：全局要求动作、事实与明确因果优先，每段只保留少量有效氛围描写，禁止同义形容词堆叠和重复渲染情绪；各世界仍保留必要的题材词汇。
- [x] **暗色模式**：界面右上角提供明/暗主题切换按钮（图标取自 lucide 图标库），主题选择本地记忆，刷新后保持。
- [x] **手机端品牌标识**：移动端顶部左上角显示「共叙」字样，与电脑端一致。
- [x] **后台故事导出**：管理界面中，每个房间/历史记录均提供导出功能，可将该局故事导出为 Markdown 文件。
- [ ] **后台继续故事**：管理界面提供「继续」功能，可恢复续玩一个历史/已结束的故事（重建房间并加载原进度）。
- [ ] **按房间诊断日志**：每局游戏单独生成结构化日志文件，按时间记录玩家事件、房间阶段与回合、服务端请求、AI 请求耗时与结果、状态推进以及完整错误原因，便于在游戏结束后复盘故障；日志须对密码、Cookie、JWT、API Key 等敏感信息脱敏。进行中的房间日志不得自动清理，房间结束后默认仅保留最近 3 局，并同时设置单文件大小上限；保留局数和大小上限均可配置，管理员可在后台查看、下载和手动删除。
- [x] **双人实时聊天胶囊**：游戏页面最下方提供悬浮胶囊聊天窗口，两位玩家可实时收发纯文字。聊天使用独立 Socket 事件和局部渲染，不进入 AI 上下文、世界书扫描或故事导出，也不会触发回合页面重绘；进行中房间的最近 1000 条聊天随房间快照持久保存，断线重连及服务重启恢复房间时一并恢复，房间结局、关闭或离线超时结束时全部清除。
