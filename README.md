...
- [ ] **开局角色塑造**：世界观背景展示之后，玩家可填写个人资料（性别、性格等，可选填或自由输入），并支持重新设置昵称——入场时填写的昵称仅作为身份标识，开局后可改名，此后在剧情中以新的个人资料示人。
- [ ] **叙事排版优化**：AI 输出的世界观背景与剧情叙事适当分段（按场景/段落自然断行），并对重点文字（关键词、专有名词、强调内容）加粗显示，前端渲染支持 Markdown 粗体。
- [ ] **暗色模式**：界面右上角提供明/暗主题切换按钮（图标取自 lucide 图标库），主题选择本地记忆，刷新后保持。
- [ ] **手机端品牌标识**：移动端顶部左上角显示「共叙」字样，与电脑端一致（当前手机端该位置未显示品牌名）。
